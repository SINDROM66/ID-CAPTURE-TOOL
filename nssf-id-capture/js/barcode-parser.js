(function (global) {
  ...
  // roughly 2-3 pixels per narrow module. Larger is NOT better ΓÇö on the
   ...
   *
   * Note: no canonical-form check. Real cards carry non-canonical base64 ΓÇö
   * the final group's unused low bits are not always zero ΓÇö so re-encoding
   ...
   * a secondary ink feature ΓÇö the MRZ on the Uganda ID back ΓÇö has higher
  ...
  // Histogram stretch ΓÇö nearest JS equivalent of PIL's ImageOps.autocontrast.
  ...
  // stand-in for PIL's LANCZOS upscaling but not identical ΓÇö if the 2x
  ...
  /**
   * Yield (label, canvas) for 0┬░, 90┬░, 180┬░, 270┬░ ΓÇö matching the
  ...
  function readBarcode(canvas) {
    ...
    try {
      ...
      // Sanitize: remove null bytes and non-printable control characters which sometimes appear as noise
      text = text.replace(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x9F]/g, "");
      // Replace weird characters that Tesseract or blurry barcode might hallucinate with spaces if not part of valid base64/date set
      // The barcode is semicolon delimited. Valid chars: A-Za-z0-9+/=; and spaces/hyphens for plaintext fallback
  ...
  async function scanCardImage(source, { debug = false } = {}) {
    ...
    for (const [regionLabel, region] of regions) {
      for (const [variantLabel, candidate] of variants(region)) {
        // Try all 4 orientations per variant ΓÇö mirrors zxingcpp try_rotate=True.
  ...
  global.UgIdParser = {
    ...
    parseNin,
    parseCard,
    // scanning (async ΓÇö all touch the camera/canvas pipeline)
    findSymbolBbox,
    findAllSymbolBboxes,
