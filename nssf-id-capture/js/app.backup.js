...
async function runOCR() {
  ...
  try {
    ...
    document.getElementById('form-alert').innerHTML = alert('error',
      'OCR failed: ' + err.message + '. Please fill the form manually.');
    fillForm({});
  }
...
function scoreOCRText(text, side) {
  ...
  if (/(C[MF]\d{9,10}[A-Z0-9]{2,4})/.test(up.replace(/[^A-Z0-9]/g, ''))) score += 25;
  return score;
}

// ─── Fix 2: Strict validators + strict extraction rules ────────────────
const NIN_REGEX = /^C[MF]\d{9,10}[A-Z0-9]{2,4}$/;
...
function parseFront(raw) {
  ...
  // NIN: strict regex
  // Search for any candidate string that fits roughly, then validate strictly.
  const compact = up.replace(/[^A-Z0-9]/g, '');
  const ninCandidate = compact.match(/C[MF][A-Z0-9]{9,14}/)?.[0] || '';
  const nin = validateNin(ninCandidate);
  if (nin) data.nin = nin;
  ...
  // Sex strict
  // Accept exact M/F only
  const sexMatch = up.match(/\b(SEX)?\s*[:\s]*([MF])\b/);
  const sex = sexMatch ? validateSexOrBlank(sexMatch[2]) : validateSexOrBlank(lines.find(l => /^[MF]$/.test(l.trim().toUpperCase())));
  if (sex) data.sex = sex;

  // Names: strip digits, strip label words, uppercase only
  // Prefer labeled region extraction if present.
  const surnamePos = up.indexOf('SURNAME');
  const givenPos = up.indexOf('GIVEN');
  if (surnamePos >= 0) {
    const slice = up.slice(surnamePos, surnamePos + 80);
    const candidate = slice.replace(/SURNAME/,' ');
    const nm = normalizeNameStrict(candidate);
    // Use first token as surname
    const tok = nm.split(/\s+/).filter(Boolean)[0] || '';
    if (isPersonNameStrict(tok)) data.surname = tok;
  }
  if (givenPos >= 0) {
    const slice = up.slice(givenPos, givenPos + 100);
    // remove GIVEN/NAME labels then take first 2-3 tokens
    const nm = normalizeNameStrict(slice);
    const toks = nm.split(/\s+/).filter(Boolean);
    if (toks.length) data.given_names = toks.slice(0, 3).join(' ');
  }
...
function parseBack(raw) {
  ...
  // If still missing, use first non-MRZ lines as fallback but strictly clean/validate.
  if (!data.village && nonMrzLines[0]) data.village = cleanLocationNameStrict(nonMrzLines[0]);
  if (!data.parish && nonMrzLines[1]) data.parish = cleanLocationNameStrict(nonMrzLines[1]);
  if (!data.district && nonMrzLines[3]) data.district = cleanLocationNameStrict(nonMrzLines[3]);
...
function mrzYYMMDDToDisplay(yyMMdd) {
...
function parseMRZ(text) {
  ...
  // Find lines matching /^[A-Z0-9<]{20,}$/
  const lines = rawLines
    .map(l => l.replace(/\s+/g, '').replace(/[^A-Z0-9<]/g, ''))
    .filter(l => /^[A-Z0-9<]{20,}$/.test(l));

  // 1) Find line starting with IDUGA → extract NIN from characters 5–19
  const idugaIdx = lines[0] ? (lines[0].indexOf('IDUGA')) : -1;
  const line1Shifted = idugaIdx >= 0 ? lines[0].slice(idugaIdx) : null;
  if (line1Shifted && line1Shifted.length >= 19) {
    const ninRaw = line1Shifted.slice(5, 19);
    const nin = validateNin(ninRaw);
    if (nin) data.nin = nin;
    data.nationality = 'UGA';
  }


  // 2) Second MRZ line → DOB from positions 0–5 (YYMMDD), sex from position 7
  const second = lines[1];
  if (second && second.length >= 8) {
    const dobYYMMDD = second.slice(0, 6);
    const dob = mrzYYMMDDToDisplay(dobYYMMDD);
    if (dob) data.dob = dob;

    const sex = second.charAt(7);
    const sexValidated = validateSexOrBlank(sex);
    if (sexValidated) data.sex = sexValidated;

    data.nationality = 'UGA';
  }

  // 3) Third MRZ line: split on << ; left=surname, right=given names; replace < with space
  const third = lines[2];
  if (third) {
    ...
    if (parts.length >= 2) {
      ...
      const surnameRaw = left.replace(/</g, ' ').replace(/\s+/g, ' ').trim();
      const givenRaw = right.replace(/</g, ' ').replace(/\s+/g, ' ').trim();

      const surname = normalizeNameStrict(surnameRaw);
      const given = normalizeNameStrict(givenRaw);

      if (isPersonNameStrict(surname)) data.surname = surname;
      if (given && isPersonNameStrict(given)) data.given_names = given;
      if (given && !isPersonNameStrict(given)) {
        // allow given names even if multiple tokens; take normalized and validate tokens
        const toks = given.split(/\s+/).filter(Boolean);
        if (toks.length) data.given_names = toks.join(' ');
      }
    }
  }
